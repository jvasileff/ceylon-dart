...
// FIXME Dart workaround; our className()s are simplistic
    check(className(1)=="Integer", "natural classname");
    check(className(1.0)=="Float", "float classname");
    check("String" == className("hello"), "string classname [1] " + className("hello"));
    check("String" == className(""), "string classname [2] " + className(""));
    check(className(' ')=="Character", "character classname");
    check(className(1->"hello").startsWith("Entry"), "entry classname");
    if (runtime.name == "jvm") {
