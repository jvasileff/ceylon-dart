import "dart:core" as $dart$core;
import "../../../../../../ceylon-dart/ceylon-dart-language/ceylon.language.dart" as $ceylon$language;

$dart$core.String dartString($dart$core.String string) => string;
$dart$core.String $package$dartString($dart$core.String string) => dartString(string);

$dart$core.int dartInt($dart$core.int integer) => integer;
$dart$core.int $package$dartInt($dart$core.int integer) => dartInt(integer);

$dart$core.double dartDouble($dart$core.double float) => float;
$dart$core.double $package$dartDouble($dart$core.double float) => dartDouble(float);

$dart$core.bool dartBool($dart$core.bool boolean) => boolean;
$dart$core.bool $package$dartBool($dart$core.bool boolean) => dartBool(boolean);

$dart$core.bool ceylonBoolean($dart$core.bool boolean) => boolean;
$dart$core.bool $package$ceylonBoolean($dart$core.bool boolean) => ceylonBoolean(boolean);

$dart$core.Type runtimeType($dart$core.Object obj) {
  return obj.runtimeType;
}
